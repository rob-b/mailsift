...
import           Network.AWS.S3                 (BucketName, ObjectKey (ObjectKey),
                                                 Region (Ireland), getObject, putObject)
...
uploadFiles
  :: (Foldable t1)
  => Pool SqlBackend
  -> TBQueue Queue.Action
  -> Env
  -> Key E.Mail
  -> t1 (t, FileInfo ByteString)
  -> IO ()
uploadFiles pool queue env key things =
  forM_ things $ \thing -> void $ uploadFile pool queue env key thing


uploadFile :: Pool SqlBackend
           -> TBQueue Queue.Action
           -> Env
           -> Key E.Mail
           -> (t, FileInfo ByteString)
           -> IO ()
uploadFile pool queue env key (_, fInfo) = do
  let dbAction = uploadAndSave env key (mkname key fInfo) (fileContent fInfo)
  _ <- Queue.add queue $ void (E.runSQLAction pool (`E.run2` dbAction))
  pure ()
  where
    mkname key' fInfo' =
